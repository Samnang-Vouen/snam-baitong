  ...
  const fetchData = async () => {
    try {
      ...
      const [cropsRes, farmersRes] = await Promise.all([
        getPlants({ includeLatest: true }).catch(err => ({ data: [] })),
        getFarmers().catch(err => ({ data: [] }))
      ]);
  ...
  return (
    <>
      <nav className="navbar">
        <div className="container-fluid" style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'nowrap', padding: '0.5rem 1rem' }}>
          <Link to="/" className="navbar-brand d-flex align-items-center" style={{ textDecoration: 'none', flex: '0 0 auto' }}>
           <img 
            src={logoAgri} 
            alt="Logo" 
          ...
          <div className="navbar-buttons-container" style={{ display: 'flex', flexDirection: 'row', gap: '0.5rem', alignItems: 'center', flexWrap: 'nowrap', marginLeft: 'auto', paddingRight: '0.5rem' }}>
            <button 
      ...
      <div className="container mt-4">
        {showManage ? (
          ...
          <FarmerDetail farmerId={selectedFarmerId} onBack={() => setShowFarmerDetail(false)} onFeedbackViewed={handleFeedbackViewed} />
        ) : (
          <>
