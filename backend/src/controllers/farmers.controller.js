...
async function generateFarmerQR(req, res) {
  try {
    ...
    // Return token for QR generation
    // Use QR_BASE_URL from env or construct from request
    const baseUrl = process.env.QR_BASE_URL || `${req.protocol}://${req.get('host')}`;
    
    // Log for debugging
    console.log('[QR Generation] Base URL:', baseUrl);
    console.log('[QR Generation] QR_BASE_URL from env:', process.env.QR_BASE_URL);
